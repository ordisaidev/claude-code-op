...
function bh(file) { return `bash "${path.join(HOOKS_DIR, file)}"`; }
...
const OUR_HOOKS = {
  ...
  PostToolUse: [{
    ...
    hooks: [
      { type:"command", command: bh('crg-update.sh'), timeout:30, statusMessage:"Updating code graph..." },
      { type:"command", command: `${MEM_PREFIX} node "$_R/scripts/bun-runner.js" "$_R/scripts/worker-service.cjs" hook claude-code observation 2>/dev/null || true`, timeout:30, statusMessage:"Saving memory observation..." }
    ]
  }],
  PreToolUse: [
